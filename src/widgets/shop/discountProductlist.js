...
class Discountproductlist extends Component {
    constructor(props) {
        super(props);
                        ...
                        <Row className="align-items-center">
                            <div className="col-lg-4 col-md-5">
                                ...
                                <Link className="card-img-hover d-block" to={`/product-single/${productdata.id}`}>
                                    <img className="card-img-top card-img-back" src={`${imgUrl}storage/product/${productdata.images[0]}`} alt="hello" />
                                    <img className="card-img-top card-img-front" src={`${imgUrl}storage/product/thumbnail/${productdata.thumbnail}`} alt="hello" />
                                </Link>
                            </div>
                            <div className="col-lg-8 col-md-7">
                                <div className="card-info">
                                    <div className="card-body">
                                        <div className="product-title"><Link to={`/product-single/${productdata.id}`} className="link-title">{productdata.name}</Link>
                                        </div>
                                        <div className="mt-1">
                                            <span className="product-price">

                                                {
                                                    productdata.discount > 0 ? productdata.discount_type == 'percent' ? <> Discount:{productdata.discount}%</> : null : null
                                                }

                                                {
                                                    productdata.discount > 0 ? productdata.discount_type == 'flat' ? <> Discount:${Math.round((productdata.discount))}off</> : null : null
                                                }
                                            </span><br />
                                            <span className="product-price">
                                                {
                                                    productdata.discount > 0 ? productdata.discount_type == 'percent' ? <>{(Math.round(productdata.unit_price, 2)) - (Math.round((productdata.unit_price * productdata.discount), 2) / 100)}</> : <>{(Math.round(productdata.unit_price, 2)) - (Math.round((productdata.discount), 2))}</> : <>${Math.round(productdata.unit_price, 2)}</>
                                                }

                                                {
                                                    productdata.discount > 0 ? <del className="text-muted h6"> ${Math.round(productdata.unit_price, 2)}</del>
                                                        : null
                                                }
                        ...
                        <Row className="align-items-center">
                            <div className="col-lg-5 col-12">
                                {(viewproduct.pictures && viewproduct.pictures.length > 0) ?
                                    ...
                                    <img className="img-fluid rounded" src={`${imgUrl}storage/product/${productdata.images[0]}`} alt="hello" />
                                    : null}
                            </div>
