...
import UserMenu from '../account/User/UserMenu';
...
class wishlist extends Component {
  ...
  RemoveItem = (Index) => {
    ...
    toast.success("Delect Wishlist Product");
  }
                      ...
                      <Row>
                        <div className="col-lg-12">
                            ...
                            <table className="cart-table table mb-0">
                              <thead>
                                <tr>
                                  ...
                                  <th className="text-left" scope="col">Discount</th>
                                ...
                                {this.GetWishlistItems()?.map((CartItem, index) => (
                                  <tr>
                                    ...
                                    <td> <span className="product-price text-muted">
                                      ৳{Math.round((CartItem.Rate * CartItem.Qty).toLocaleString(navigator.language, { minimumFractionDigits: 0 }) / convert, 0)}
                                    </span>
                                    </td>
                                    <td> <span className="product-price text-muted">
                                      ৳{Math.round((CartItem.Discount * CartItem.Qty).toLocaleString(navigator.language, { minimumFractionDigits: 0 }) / convert, 0)}
                                    </span>
                                    </td>
                                    <td className="text-right">
                                      {!this.CartItems(CartItem.ProductID) ?
                                        <Link to="#" onClick={() => this.Productaddcart(CartItem.ProductID, CartItem.ProductName, CartItem.ProductImage, 1, CartItem.Rate, CartItem.Discount, "In Stock")} className="btn-cart btn btn-primary btn-animated mx-3" rel="nofollow" id="addtocard"><i className="las la-shopping-cart mr-1" /></Link>
                                        :
                                        <Link to="/OneStepCheck" className="btn-cart btn btn-primary btn-animated ml-3" rel="nofollow" id="viewcart"><i className="las la-cart-plus mr-1" /></Link>
                                      }<br />
                                      <Link type="submit" className="btn btn-primary btn-animated mt-3" onClick={() => this.RemoveItem(index)}><i className="las la-trash-alt" />
                                      </Link>
                          ...
                          <div className="d-md-flex align-items-end justify-content-end border-top pt-5">
                            <Link to="/OneStepCheck" className="btn btn-primary btn-animated mt-3 mt-md-0">View Cart</Link>
                          </div>
                        </div>
                      </Row>
                      :
